...
def run_super_res_process(midasZarrDir: str, srfac: int, SRconfig_path: str, midasVerDir: str, outf: str = 'SROut.txt', errf: str = 'SRErr.txt'):

    cmd = f"{pytpath} {os.path.join(midasVerDir, 'super_res_process.py')} -midasZarrDir {midasZarrDir} -srfac {srfac} -SRconfig_path {SRconfig_path} -midasVerDir {midasVerDir}"
    outf_path = f"{midasZarrDir}/output/{outf}"
    errf_path = f"{midasZarrDir}/output/{errf}"

    try:
        safely_run_command(cmd, midasZarrDir, outf_path, errf_path, task_name="SR workflow")

    except Exception as e:
        logger.error(f"Failed to executre DR workflow: {e}")
        return None
...
def process_layer(layer_nr: int, top_res_dir: str, ps_fn: str, data_fn: str, num_procs: int, 
                 ...
                 runSR: int = 0, srfac: int = 8, SRconfig_path: str = 'sr_config.json') -> None:
    ...
    if provide_input_all == 0:
        if do_peak_search == 1:
            ...
            if "/" not in SRconfig_path:
                SRconfig_path = os.path.join(v7_dir, SRconfig_path)
            run_super_res_process(result_dir, srfac, SRconfig_path, v7_dir, ps_fn)
...
def main():
    ...
    parser.add_argument('-runSR', type=int, required=False, default=0, 
                        help='If want to run super resolution, put to 1.')
    parser.add_argument('-srfac', type=int, required=False, default=8, 
                        help='Super resolution factor (needed only if -runSR set to 1).')
    parser.add_argument('-SRconfig_path', type=str, required=False, default='sr_config.json', 
                        help='Path to the super resolution configuration file.')
